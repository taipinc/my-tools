...
const DEFAULT_HDR = `${import.meta.env.BASE_URL}hdr/penguin_museum_2k.hdr`;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      <div
        ...
        style={{ position: "relative", height: 112, margin: "0 26px", touchAction: "none", cursor: "ew-resize", outline: "none" }}
      >
        ...
        {labels.map((l, i) => {
          ...
          return (
            <div key={i} style={{ position: "absolute", top: 0, bottom: 0, width: 0, left: `${(i / (n - 1)) * 100}%` }}>
              <div style={{ position: "absolute", left: -1, bottom: 42, width: 2, height: full ? 36 : 18, background: full ? INK : "#93a1b8" }} />
              {full && (
                <div style={{ position: "absolute", top: 80, left: 0, transform: "translateX(-50%)", fontFamily: MONO, fontSize: 11, color: INK_SOFT, whiteSpace: "nowrap" }}>{l}</div>
              )}
            </div>
          );
        })}
        <div style={{ position: "absolute", top: 0, bottom: 0, width: 0, left: `${(index / (n - 1)) * 100}%` }}>
          <div style={{ position: "absolute", left: -1, bottom: 42, width: 2, height: 60, background: MARK }} />
          <div style={{ position: "absolute", bottom: 32, left: 0, width: 20, height: 20, marginLeft: -10, borderRadius: "50%", background: "#fff", border: `2.5px solid ${INK}`, boxShadow: "0 1px 3px rgba(28,40,64,0.18)" }} />
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  return (
    <div style={{ width: "100%", height: "100%", minHeight: 720, display: "flex", alignItems: "stretch", overflow: "auto", color: INK, userSelect: "none", WebkitUserSelect: "none" }}>
      <div style={{ flex: 1, padding: "34px 32px 28px", display: "flex", flexDirection: "column", minWidth: 900 }}>
      ...
      <div style={{ width: 384, flex: "0 0 384px", padding: "34px 28px 28px 0", display: "flex", flexDirection: "column", gap: 16 }}>
